    ...
    async def validate_schema_with_debug(
        ...
        try:
            ...
            if 'cursor' in locals() and cursor:
                try:
                    cursor.close()
                    debug_log("validate_schema_with_debug: 커서 닫기 완료")
                except Exception as e:
                    debug_log(f"validate_schema_with_debug: 커서 닫기 오류: {e}")

            if 'connection' in locals() and connection and connection.is_connected():
                try:
                    connection.close()
                    debug_log("validate_schema_with_debug: 연결 닫기 완료")
                except Exception as e:
                    debug_log(f"validate_schema_with_debug: 연결 닫기 오류: {e}")

            # SSH 터널 정리
            if 'tunnel_used' in locals() and tunnel_used:
                try:
                    self.cleanup_ssh_tunnel()
                    debug_log("validate_schema_with_debug: SSH 터널 정리 완료")
                except Exception as e:
                    debug_log(f"validate_schema_with_debug: SSH 터널 정리 오류: {e}")
