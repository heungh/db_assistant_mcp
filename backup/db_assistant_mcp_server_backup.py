#!/usr/bin/env python3
"""
DB Assistant Amazon Q CLI MCP 서버 (간소화 버전)
"""
...
class DBAssistantMCPServer:
    def __init__(self):
        ...
        self.table_name = 'execution-plans'
    ...
    def _ensure_table_exists(self):
        ...
        try:
            ...
            table.load()
            print(f"기존 테이블 {self.table_name} 사용")
        except ClientError as e:
            if e.response['Error']['Code'] == 'ResourceNotFoundException':
                print(f"테이블 {self.table_name} 생성 중...")
    ...
    def _get_plan_from_dynamodb(self, operation: str) -> Optional[Dict[str, Any]]:
        """DynamoDB에서 실행 계획 조회"""
        try:
            table = self.dynamodb.Table(self.table_name)
            response = table.get_item(Key={'operation': operation})
            return response.get('Item')
        except Exception as e:
            print(f"계획 조회 실패: {e}")
            return None

    def _save_plan_to_dynamodb(self, operation: str, plan_data: Dict[str, Any]):
        """DynamoDB에 실행 계획 저장"""
        try:
            table = self.dynamodb.Table(self.table_name)
            table.put_item(Item={
    ...
    async def validate_ddl(self, ddl_content: str, database_secret: Optional[str], filename: str) -> str:
        ...
        try:
            ...
            if schema_validation and schema_validation["success"]:
                ...
                """
                for result in schema_validation["validation_results"]:
                    status_class = "success" if result["exists"] and not result["column_issues"] else "error"
            ...
            if constraint_validation and constraint_validation["success"]:
                ...
                for result in constraint_validation["constraint_results"]:
                    ...
                    if result["issue"]:
                        constraint_section += f"<p class='error-message'>{result['issue']}</p>"
...
<body>
    ...
    async def list_databases(self, database_secret: str, use_ssh_tunnel: bool = True):
        """데이터베이스 목록 조회 및 선택"""
        if mysql is None:
    ...
    async def select_database(self, database_secret: str, database_selection: str, use_ssh_tunnel: bool = True):
        """데이터베이스 선택 및 연결 테스트"""
        try:
            # 먼저 데이터베이스 목록을 가져옴
            db_info = await self.list_databases(database_secret, use_ssh_tunnel)
            ...
            # 선택된 데이터베이스로 연결 테스트
            connection, tunnel_used = await self.get_db_connection(database_secret, selected_db, use_ssh_tunnel)
            ...
            if connection.is_connected():
                ...
                result = f"✅ 데이터베이스 '{selected_db}' 연결 성공!\n\n"
                result += f"🔗 연결 정보:\n"
                result += f"• 현재 데이터베이스: {current_db}\n"
                result += f"• MySQL 버전: {version}\n"
                result += f"• SSH 터널 사용: {'예' if tunnel_used else '아니오'}\n"
                
                return result
            else:
                return f"❌ 데이터베이스 '{selected_db}' 연결 실패"
    ...
    async def test_database_connection(self, database_secret: str, use_ssh_tunnel: bool = True) -> Dict[str, Any]:
        """데이터베이스 연결 테스트"""
        try:
            connection, tunnel_used = await self.get_db_connection(database_secret, use_ssh_tunnel)
    ...
    async def analyze_current_schema(self, database_secret: str, use_ssh_tunnel: bool = True) -> Dict[str, Any]:
        """현재 데이터베이스 스키마 상세 분석"""
        return await self._analyze_current_schema_original(database_secret, use_ssh_tunnel)

    async def _analyze_current_schema_original(self, database_secret: str, use_ssh_tunnel: bool = True) -> Dict[str, Any]:
        """원본 스키마 분석 로직"""
        try:
            ...
            for table_info in tables_info:
                ...
                for idx_info in indexes_info:
                    ...
                    if current_index != idx_name:
                        ...
                        # 복합 인덱스의 추가 컬럼
                        schema_analysis["tables"][table_name]["indexes"][-1]["columns"].append(idx_info[1])
            ...
            return {
                "success": False,
                "error": f"스키마 분석 오류: {str(e)}"
            }

    async def check_ddl_conflicts(self, ddl_content: str, database_secret: str, use_ssh_tunnel: bool = True) -> Dict[str, Any]:
        """DDL 실행 전 충돌 및 문제점 사전 검사"""
        try:
            # 현재 스키마 분석
            schema_result = await self.analyze_current_schema(database_secret, use_ssh_tunnel)
            ...
            if ddl_type == "CREATE_TABLE":
                conflicts.extend(await self._check_create_table_conflicts(ddl_content, schema))
            elif ddl_type == "ALTER_TABLE":
                conflicts.extend(await self._check_alter_table_conflicts(ddl_content, schema))
            elif ddl_type == "CREATE_INDEX":
                conflicts.extend(await self._check_create_index_conflicts(ddl_content, schema))
            elif ddl_type == "DROP":
            ...
            return {
                "success": False,
                "error": f"DDL 충돌 검사 오류: {str(e)}"
            }

    async def _check_create_table_conflicts(self, ddl_content: str, schema: Dict) -> List[str]:
        """CREATE TABLE 충돌 검사"""
        conflicts = []
        
        # 테이블명 추출
        table_match = re.search(r'CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?`?(\w+)`?', ddl_content, re.IGNORECASE)
        if table_match:
            ...
            if table_name in schema["tables"]:
                if "IF NOT EXISTS" not in ddl_content.upper():
                    conflicts.append(f"테이블 '{table_name}'이 이미 존재합니다. IF NOT EXISTS를 사용하거나 다른 이름을 선택하세요.")
                else:
                    conflicts.append(f"테이블 '{table_name}'이 이미 존재합니다. IF NOT EXISTS가 있어서 실행은 되지만 아무 작업도 수행되지 않습니다.")
        
        return conflicts

    async def _check_alter_table_conflicts(self, ddl_content: str, schema: Dict) -> List[str]:
        """ALTER TABLE 충돌 검사"""
        conflicts = []
        
        # 테이블명 추출
        table_match = re.search(r'ALTER\s+TABLE\s+`?(\w+)`?', ddl_content, re.IGNORECASE)
        if table_match:
            table_name = table_match.group(1)
            ...
            # ADD COLUMN 검사
            add_column_matches = re.findall(r'ADD\s+(?:COLUMN\s+)?`?(\w+)`?', ddl_content, re.IGNORECASE)
            for col_name in add_column_matches:
                if col_name in table_info["columns"]:
                    conflicts.append(f"컬럼 '{col_name}'이 테이블 '{table_name}'에 이미 존재합니다.")
            
            # DROP COLUMN 검사
            drop_column_matches = re.findall(r'DROP\s+(?:COLUMN\s+)?`?(\w+)`?', ddl_content, re.IGNORECASE)
            for col_name in drop_column_matches:
                if col_name not in table_info["columns"]:
                    conflicts.append(f"컬럼 '{col_name}'이 테이블 '{table_name}'에 존재하지 않습니다.")
                else:
                    # 외래키 참조 확인
                    for fk in table_info["foreign_keys"]:
                        if fk["column"] == col_name:
                            conflicts.append(f"컬럼 '{col_name}'은 외래키로 사용 중이므로 삭제할 수 없습니다.")
            
            # MODIFY/CHANGE COLUMN 검사
            modify_matches = re.findall(r'(?:MODIFY|CHANGE)\s+(?:COLUMN\s+)?`?(\w+)`?', ddl_content, re.IGNORECASE)
            for col_name in modify_matches:
                if col_name not in table_info["columns"]:
                    conflicts.append(f"수정하려는 컬럼 '{col_name}'이 테이블 '{table_name}'에 존재하지 않습니다.")
        
        return conflicts

    async def _check_create_index_conflicts(self, ddl_content: str, schema: Dict) -> List[str]:
        """CREATE INDEX 충돌 검사"""
        conflicts = []
        
        # 인덱스명과 테이블명 추출
        index_match = re.search(r'CREATE\s+(?:UNIQUE\s+)?INDEX\s+`?(\w+)`?\s+ON\s+`?(\w+)`?\s*\((.*?)\)', ddl_content, re.IGNORECASE)
        if index_match:
    ...
    async def _check_drop_conflicts(self, ddl_content: str, schema: Dict) -> List[str]:
        ...
        if "DROP TABLE" in ddl_content.upper():
            # 테이블 삭제 검사
            table_match = re.search(r'DROP\s+TABLE\s+(?:IF\s+EXISTS\s+)?`?(\w+)`?', ddl_content, re.IGNORECASE)
            if table_match:
                table_name = table_match.group(1)
                if table_name not in schema["tables"]:
                    if "IF EXISTS" not in ddl_content.upper():
                        conflicts.append(f"삭제하려는 테이블 '{table_name}'이 존재하지 않습니다.")
                else:
                    # 외래키 참조 확인 (다른 테이블에서 이 테이블을 참조하는지)
                    for other_table, table_info in schema["tables"].items():
                        for fk in table_info["foreign_keys"]:
                            if fk["referenced_table"] == table_name:
                                conflicts.append(f"테이블 '{table_name}'은 테이블 '{other_table}'에서 외래키로 참조되고 있어 삭제할 수 없습니다.")
        
        elif "DROP INDEX" in ddl_content.upper():
            # 인덱스 삭제 검사
            index_match = re.search(r'DROP\s+INDEX\s+`?(\w+)`?\s+ON\s+`?(\w+)`?', ddl_content, re.IGNORECASE)
            if index_match:
                index_name = index_match.group(1)
                table_name = index_match.group(2)
                ...
                if table_name not in schema["tables"]:
                    ...
                    table_info = schema["tables"][table_name]
                    index_exists = any(idx["name"] == index_name for idx in table_info["indexes"])
                    if not index_exists:
                        conflicts.append(f"인덱스 '{index_name}'이 테이블 '{table_name}'에 존재하지 않습니다.")
        
        return conflicts

    async def get_aurora_mysql_parameters(self, cluster_identifier: str, region: str = "ap-northeast-2", 
                                        filter_type: str = "important", category: str = "all") -> str:
        ...
        try:
            ...
            cluster_param_group = cluster.get('DBClusterParameterGroup', 'default.aurora-mysql8.0')
...
- 엔진 버전: {cluster.get('EngineVersion', 'N/A')}"""
            ...
            # 클러스터 파라미터 조회
            cluster_params = await self._get_parameters(rds_client, cluster_param_group, 'cluster', filter_type, category)
            if cluster_params:
                result += f"\n\n🏗️ **클러스터 레벨 파라미터:**\n{cluster_params}"
            ...
            if cluster.get('DBClusterMembers'):
                ...
                if instance_response['DBInstances']:
                    instance_param_group = instance_response['DBInstances'][0]['DBParameterGroups'][0]['DBParameterGroupName']
                    result += f"\n- 인스턴스 파라미터 그룹: {instance_param_group}"
                    
                    instance_params = await self._get_parameters(rds_client, instance_param_group, 'instance', filter_type, category)
                    if instance_params:
                        result += f"\n\n🖥️ **인스턴스 레벨 파라미터:**\n{instance_params}"
                    else:
                        if category == "all":
    ...
    async def _get_parameters(self, rds_client, param_group_name: str, level: str, filter_type: str, category: str) -> str:
        """파라미터 조회 및 필터링"""
        try:
            # 파라미터 조회
            if level == 'cluster':
                response = rds_client.describe_db_cluster_parameters(DBClusterParameterGroupName=param_group_name)
            else:
                response = rds_client.describe_db_parameters(DBParameterGroupName=param_group_name)
            ...
            category_params = {
                'security': [
                    'activate_all_roles_on_login', 'authentication_kerberos_caseins_cmp', 
                    'default_authentication_plugin', 'default_password_lifetime',
                    'check_proxy_users', 'mysql_native_password_proxy_users',
                    'sha256_password_proxy_users', 'validate_password_policy'
                ],
                'performance': [
                    'innodb_thread_concurrency', 'innodb_read_io_threads', 'innodb_write_io_threads',
                    'thread_cache_size', 'thread_stack', 'innodb_purge_threads',
                    'innodb_adaptive_flushing', 'innodb_adaptive_max_sleep_delay',
                    'innodb_concurrency_tickets', 'innodb_flushing_avg_loops',
                    ...
                    'innodb_parallel_read_threads', 'query_cache_size', 'query_cache_type'
                ],
                'memory': [
                    'innodb_buffer_pool_size', 'tmp_table_size', 'max_heap_table_size',
                    'sort_buffer_size', 'read_buffer_size', 'read_rnd_buffer_size', 
                    'join_buffer_size', 'innodb_log_buffer_size', 'key_buffer_size',
                    ...
                    'innodb_change_buffer_max_size', 'bulk_insert_buffer_size'
                ],
                'io': [
                    'innodb_flush_log_at_trx_commit', 'sync_binlog', 'innodb_log_file_size',
                    'innodb_io_capacity', 'innodb_io_capacity_max', 'innodb_flush_method',
                    'innodb_file_per_table', 'innodb_doublewrite', 'innodb_flush_neighbors',
                    'innodb_flush_log_at_timeout', 'innodb_log_compressed_pages',
                    'innodb_open_files', 'innodb_read_only', 'innodb_sort_buffer_size'
                ],
                'connection': [
                    'max_connections', 'max_user_connections', 'connect_timeout',
                    'interactive_timeout', 'wait_timeout', 'max_connect_errors',
                    'back_log', 'host_cache_size', 'max_allowed_packet'
                ],
                'logging': [
                    'general_log', 'slow_query_log', 'log_queries_not_using_indexes',
                    'long_query_time', 'log_slow_admin_statements', 'log_slow_slave_statements',
                    'general_log_file', 'slow_query_log_file', 'log_error',
                    'log_warnings', 'innodb_print_all_deadlocks'
                ],
                'replication': [
                    'binlog_format', 'expire_logs_days', 'max_binlog_size', 'binlog_cache_size',
                    'slave_net_timeout', 'slave_parallel_workers', 'binlog_checksum',
                    'binlog_group_commit_sync_delay', 'binlog_group_commit_sync_no_delay_count',
                    'binlog_order_commits', 'binlog_row_image', 'binlog_rows_query_log_events',
                    'binlog_stmt_cache_size', 'binlog_transaction_compression',
                    'binlog_backup', 'binlog_replication_globaldb'
                ],
                'aurora': [
                    'aurora_binlog_replication_max_yield_seconds', 'aurora_binlog_replication_sec_index_parallel_workers',
                    'aurora_enable_staggered_replica_restart', 'aurora_enhanced_binlog',
                    'aurora_full_double_precision_in_json', 'aurora_fwd_writer_idle_timeout',
                    'aurora_fwd_writer_max_connections_pct', 'aurora_in_memory_relaylog',
                    'aurora_jemalloc_background_thread', 'aurora_jemalloc_dirty_decay_ms',
                    'aurora_jemalloc_tcache_enabled', 'aurora_ml_inference_timeout',
                    'aurora_oom_response', 'aurora_parallel_query', 'aurora_read_replica_read_committed',
                    'aurora_replica_read_consistency', 'aurora_tmptable_enable_per_table_limit',
                    'aurora_use_vector_instructions', 'aurora_aurora_max_partitions_for_range'
                ]
            }
            ...
            if filter_type == "all":
                ...
                important_params = [
                    'innodb_buffer_pool_size', 'max_connections', 'innodb_log_file_size',
                    ...
                    'aurora_parallel_query', 'aurora_oom_response', 'aws_default_s3_role'
                ]
                filtered_params = [p for p in parameters if p['ParameterName'] in important_params]
            
            # 카테고리 필터링
            if category != "all" and category in category_params:
                category_param_names = category_params[category]
                filtered_params = [p for p in filtered_params if p['ParameterName'] in category_param_names]
            ...
            if category == "all":
                ...
                for cat in ['security', 'performance', 'memory', 'io', 'connection', 'logging', 'replication', 'aurora']:
                    if cat in categorized_params:
                        ...
                        for param in categorized_params[cat]:
                            ...
                            description = param.get('Description', '')[:50] + "..." if len(param.get('Description', '')) > 50 else param.get('Description', '')
    ...
    async def _get_default_aurora_parameters(self, parameter_group_name: str, region: str) -> str:
        ...
        try:
            ...
            if not default_group:
                return f"❌ 기본 파라미터 그룹을 찾을 수 없습니다: {parameter_group_name}"
...
📋 **기본 파라미터 (일부):**"""
    ...
    async def create_execution_plan(self, operation: str, modify_plan: bool = False, **kwargs) -> Dict[str, Any]:
        """실행 계획 생성"""
        # DynamoDB 사용을 임시로 비활성화하고 직접 계획 생성
        try:
            print(f"DEBUG: create_execution_plan called with operation='{operation}', kwargs={kwargs}")
            
            # 새로운 계획 생성
            plan_steps = self._create_default_plan(operation)
            plan_data = {
                'steps': plan_steps,
                'description': f'{operation} 작업을 위한 실행 계획'
            }
            
            print(f"🆕 새로운 실행 계획 생성")
            ...
            plan = {
                ...
                "estimated_duration": f"{len(plan_steps) * 2}초"
            }
            ...
            # 계획 출력
            result = f"""📋 **실행 계획 생성 완료**

🎯 **작업:** {operation}
⏱️ **예상 소요시간:** {plan['estimated_duration']}
📅 **생성시간:** {plan['created_at']}

📝 **실행 단계:**"""
            
            for step in plan_steps:
                result += f"""
{step['step']}. {step['action']} (대상: {step['target']}, 도구: {step['tool']})"""
            
            result += f"""

⚠️ **주의사항:**
- 이 작업은 데이터베이스에 영향을 줄 수 있습니다
- 실행 전 계획을 신중히 검토해주세요

❓ **실행하시겠습니까?** (y/yes 또는 n/no로 응답)"""
            
            return {"success": True, "plan": plan, "message": result}
            
        except Exception as e:
            print(f"DEBUG: Exception in create_execution_plan: {e}")
            import traceback
            traceback.print_exc()
            return {"success": False, "error": f"실행 계획 생성 실패: {str(e)}"}

    def _create_default_plan(self, operation: str) -> List[Dict[str, Any]]:
        """기본 실행 계획 생성"""
        if operation == "validate_sql_file":
            return [
                ...
                {"step": 7, "action": "Claude AI 검증", "target": "고급 분석", "tool": "claude_analysis"},
                {"step": 8, "action": "HTML 보고서 생성", "target": "output 디렉토리", "tool": "fs_write"}
            ]
        ...
        elif "스키마 분석" in operation or "schema" in operation.lower():
            return [
                {"step": 1, "action": "데이터베이스 시크릿 조회", "target": "AWS Secrets Manager", "tool": "list_database_secrets"},
                {"step": 2, "action": "데이터베이스 목록 확인", "target": "클러스터 내 DB", "tool": "list_databases"},
                {"step": 3, "action": "데이터베이스 선택", "target": "대상 DB", "tool": "select_database"},
                {"step": 4, "action": "연결 상태 확인", "target": "connection stats", "tool": "get_connection_stats"},
                {"step": 5, "action": "테이블 I/O 통계", "target": "table performance", "tool": "get_table_io_stats"},
                {"step": 6, "action": "인덱스 사용 통계", "target": "index usage", "tool": "get_index_usage_stats"},
                {"step": 7, "action": "성능 메트릭 조회", "target": "performance data", "tool": "get_performance_metrics"},
                {"step": 8, "action": "메모리 사용량 분석", "target": "memory usage", "tool": "get_memory_usage"}
            ]
            ...
            return [
                ...
                {"step": 2, "action": "테이블 목록 조회", "target": "information_schema", "tool": "mysql_query"},
                {"step": 3, "action": "컬럼 정보 수집", "target": "각 테이블", "tool": "mysql_query"},
                {"step": 4, "action": "스키마 요약 생성", "target": "종합 정보", "tool": "internal_analysis"}
            ]
        else:
            return [
                {"step": 1, "action": f"{operation} 실행", "target": "기본 작업", "tool": "generic"}
            ]
    ...
    async def execute_with_auto_plan(self, operation: str, **kwargs) -> str:
        ...
        try:
            # 1. 실행 계획 생성
            plan_result = await self.create_execution_plan(operation, **kwargs)
            print("plan_display", plan_result, file=sys.stderr)
            
            # 계획 생성 실패 시 에러 반환
            if not plan_result.get("success", False):
                return f"❌ 실행 계획 생성 실패: {plan_result.get('error', '알 수 없는 오류')}"
            
            plan_display = self._format_plan_display(plan_result["plan"])
...
📝 **실행 단계:**"""
    ...
    async def confirm_and_execute(self, confirmation: str) -> str:
        ...
        try:
            ...
            for step in steps:
                ...
                print(f"[{step_num}/{len(steps)}] 실행 중: {action} (도구: {tool})")
                
                # 도구별 동적 실행
                step_result = await self._execute_step(tool, target, params, step_results)
                step_results[f"step_{step_num}"] = step_result
                
                result += f"✅ Step {step_num}: {action} - 완료\n"
            
            # 최종 결과 생성
            final_result = await self._generate_final_result(operation, params, step_results)
            result += f"\n{final_result}"
    ...
    async def _execute_step(self, tool: str, target: str, params: Dict[str, Any], step_results: Dict[str, Any]) -> Any:
        """단계별 도구 실행"""
        if tool == "test_database_connection":
            return await self.test_connection_only(params.get('database_secret'))
            
        elif tool == "mysql_query":
            if target == "information_schema":
                # 테이블 목록 조회
                connection, _ = await self.get_db_connection(params.get('database_secret'), self.selected_database)
                cursor = connection.cursor()
                cursor.execute("""
                    SELECT table_name, table_type, engine, table_rows, 
                           data_length, index_length, table_comment
                ...
                # 컬럼/인덱스/외래키 정보 수집
                return await self._collect_table_details(params.get('database_secret'))
                
        elif tool == "fs_read":
            if params.get('filename'):
                return await self._read_sql_file(params['filename'])
                
        elif tool == "internal_parser":
            if 'file_content' in step_results:
                return await self._parse_ddl(step_results['file_content'])
                
        elif tool == "analyze_current_schema":
            return await self.analyze_current_schema(params.get('database_secret'))
            
        elif tool == "check_ddl_conflicts":
            return await self.check_ddl_conflicts(params.get('ddl_content'), params.get('database_secret'))
            
        elif tool == "claude_analysis":
            return await self._claude_analyze(step_results)
            
        elif tool == "fs_write":
            return await self._write_report(target, step_results)
            
        elif tool == "use_aws":
            if target == "cluster_identifier":
                return await self._get_cluster_info(params.get('cluster_identifier'), params.get('region', 'ap-northeast-2'))
            elif target == "적용된 그룹":
                return await self._get_parameter_group(step_results.get('cluster_info'))
            elif target == "주요 설정":
                return await self._get_parameters(step_results.get('parameter_group'))
                
        elif tool == "internal_filter":
            return await self._filter_custom_settings(step_results.get('parameters'))
            
        elif tool == "internal_analysis":
            return await self._analyze_schema_data(step_results)
            
        elif tool == "internal_check":
            return await self._validate_conditions(target, step_results)
            
        elif tool == "list_sql_files":
            return await self.list_sql_files()
            
        elif tool == "validate_sql_file":
            return await self.validate_sql_file(params.get('filename'), params.get('database_secret'))
            
        else:
            return f"도구 '{tool}' 실행 완료"

    async def _generate_final_result(self, operation: str, params: Dict[str, Any], step_results: Dict[str, Any]) -> str:
        """최종 결과 생성"""
        if operation == "analyze_current_schema":
            schema_data = step_results.get('step_6')  # internal_analysis 결과
            if schema_data and isinstance(schema_data, dict) and schema_data.get("success"):
                schema = schema_data["schema_analysis"]
...
🔹 **{table_name}** ({table_info['engine']})
   ...
   - 예상 행 수: {table_info['rows']:,}"""
        ...
        elif operation == "validate_sql_file":
            return f"📄 SQL 파일 검증 완료"
            
        elif operation == "get_aurora_mysql_parameters":
            return f"⚙️ Aurora 파라미터 조회 완료"
            
        else:
            return f"📊 {operation} 작업 완료"

    async def _collect_table_details(self, database_secret: str) -> Dict[str, Any]:
        """테이블 상세 정보 수집"""
        connection, _ = await self.get_db_connection(database_secret, self.selected_database)
        cursor = connection.cursor()
    ...
    async def _parse_ddl(self, content: str) -> Dict[str, Any]:
        """DDL 구문 분석"""
        return {"parsed": True, "content": content}

    async def _claude_analyze(self, step_results: Dict[str, Any]) -> Dict[str, Any]:
        """Claude AI 분석"""
        return {"analysis": "Claude 분석 완료", "recommendations": 3}

    async def _write_report(self, target: str, step_results: Dict[str, Any]) -> str:
        """보고서 작성"""
        return f"보고서 저장: {target}/report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"

    async def _get_cluster_info(self, cluster_id: str, region: str) -> Dict[str, Any]:
        """클러스터 정보 조회"""
        return {"cluster_id": cluster_id, "region": region}

    async def _get_parameter_group(self, cluster_info: Dict[str, Any]) -> Dict[str, Any]:
        """파라미터 그룹 조회"""
        return {"parameter_group": "default.aurora-mysql8.0"}

    async def _get_parameters(self, param_group: Dict[str, Any]) -> Dict[str, Any]:
        """파라미터 조회"""
        return {"parameters": {"innodb_buffer_pool_size": "75%"}}

    async def _filter_custom_settings(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """커스텀 설정 필터링"""
        return {"custom_settings": 5}

    async def _analyze_schema_data(self, step_results: Dict[str, Any]) -> Dict[str, Any]:
        """스키마 데이터 분석"""
        # 원본 스키마 분석 메서드 호출
        if 'step_1' in step_results:  # DB 연결 성공
            # 실제 스키마 분석 수행
            database_secret = None
            for key, value in step_results.items():
                if isinstance(value, str) and 'gamedb1-cluster' in str(value):
                    database_secret = 'gamedb1-cluster'
                    break
            
            if database_secret:
                return await self._analyze_current_schema_original(database_secret)
            
        return {"success": False, "error": "DB 연결 실패"}

    async def _validate_conditions(self, target: str, step_results: Dict[str, Any]) -> bool:
        """조건 검증"""
        return True
    ...
    async def get_schema_summary(self, database_secret: str) -> str:
        """현재 스키마 요약 정보 반환"""
        try:
            print(f"DEBUG: get_schema_summary called with database_secret='{database_secret}'")
            
            # 직접 데이터베이스 연결해서 스키마 정보 조회
            print("DEBUG: Calling get_db_connection...")
            connection, tunnel_used = await self.get_db_connection(database_secret, self.selected_database)
            print("DEBUG: get_db_connection successful")
            ...
            tables_info = cursor.fetchall()
            print(f"DEBUG: Found {len(tables_info)} tables")
...
📋 **테이블 목록** ({len(tables_info)}개):"""
  ...
  🔹 **{table_name}** ({engine})
     ...
     - 예상 행 수: {rows:,}"""
            ...
            print("DEBUG: get_schema_summary completed successfully")
            return summary
            
        except Exception as e:
            print(f"DEBUG: Exception in get_schema_summary: {e}")
            import traceback
            traceback.print_exc()
...
{tables_list if tables_list else '   (없음)'}"""
    ...
    async def analyze_slow_queries(self, database_secret: str, limit: int = 10) -> str:
        ...
        try:
            ...
            if not slow_queries:
                ...
                for i, (pattern, count, avg_time, max_time, total_time, avg_examined, avg_sent) in enumerate(slow_queries, 1):
                    pattern_short = (pattern[:80] + "...") if len(pattern) > 80 else pattern
                    result += f"**{i}. 쿼리 패턴:**\n```sql\n{pattern_short}\n```\n"
                    result += f"📈 **통계:**\n"
    ...
    async def get_table_io_stats(self, database_secret: str, schema_name: Optional[str] = None) -> str:
        """테이블별 I/O 통계 조회"""
        try:
            connection, tunnel_used = await self.get_db_connection(database_secret)
            cursor = connection.cursor()
            ...
            if not io_stats:
                ...
                for i, (table, read_count, write_count, total_io, read_time, write_time, total_time) in enumerate(io_stats, 1):
                    result += f"**{i}. {table}**\n"
    ...
    async def get_index_usage_stats(self, database_secret: str, table_name: Optional[str] = None) -> str:
        """인덱스 사용 통계 조회"""
        try:
            connection, tunnel_used = await self.get_db_connection(database_secret)
            cursor = connection.cursor()
            ...
            if not index_stats:
                ...
                for i, (table, index, read_count, write_count, total_usage, read_time, write_time) in enumerate(index_stats, 1):
        ...
        try:
            ...
            try:
                ...
                if current_locks:
                    ...
                    for schema, table, lock_type, lock_mode, status, thread_id in current_locks:
            ...
            try:
                ...
                if lock_waits:
                    result += f"⏳ **락 대기 상황:**\n"
                    for req_thread, block_thread, schema, table, lock_type in lock_waits:
            ...
            if wait_events:
                ...
                for event, count, total_wait, avg_wait in wait_events:
                    event_short = event.replace('wait/synch/', '').replace('wait/io/', '')
    ...
    async def get_replication_status(self, database_secret: str) -> str:
        ...
        try:
            ...
            try:
                ...
                if repl_config:
                    result += f"⚙️ **복제 연결 설정:**\n"
                    for channel, host, port, user, auto_failover, retry_interval, retry_count in repl_config:
                        result += f"- 채널: {channel}\n"
                        result += f"  소스: {host}:{port} (사용자: {user})\n"
                        result += f"  자동 장애조치: {auto_failover}\n"
                        result += f"  재시도: {retry_count}회, 간격: {retry_interval}초\n\n"
                else:
            ...
            try:
                ...
                if worker_status:
                    result += f"👷 **복제 워커 상태:**\n"
                    for channel, worker_id, state, error_msg, last_trans in worker_status:
...
async def handle_list_tools() -> list[types.Tool]:
    ...
    return [
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "category": {
                        ...
                        "enum": ["all", "security", "performance", "memory", "io", "connection", "logging", "replication", "aurora"],
                        "default": "all"
                    }
                },
        ...
        types.Tool(
            ...
            description="특정 데이터베이스를 선택하여 연결합니다",
            inputSchema={
                "type": "object",
                "properties": {
                    "database_secret": {
...
async def handle_call_tool(name: str, arguments: dict) -> list[types.TextContent]:
    ...
    try:
        ...
        if name == "list_sql_files":
            ...
            # 계획 기반 실행 확인
            # 자동 계획 생성 및 실행
            result = await db_assistant.execute_with_auto_plan(
                "validate_sql_file",
            ...
            # 동적 실행 시스템 사용
            plan_result = await db_assistant.create_execution_plan(
                "analyze_current_schema",
                database_secret=arguments["database_secret"]
            )
            # 자동으로 실행
            result = await db_assistant.confirm_and_execute("y")
            ...
            try:
                # 직접 간단한 스키마 정보 조회
                connection, tunnel_used = await db_assistant.get_db_connection(
                    arguments["database_secret"], 
                    db_assistant.selected_database
                )
                cursor = connection.cursor()
                ...
                result = f"📊 데이터베이스 스키마 요약 (DB: {current_db})\n\n"
                result += f"📋 **테이블 목록** ({len(tables)}개):\n"
                
                for table_name, engine, rows in tables:
                    rows = rows or 0
                    result += f"  🔹 **{table_name}** ({engine}) - {rows:,}행\n"
                
                cursor.close()
                connection.close()
                
                if tunnel_used:
                    db_assistant.cleanup_ssh_tunnel()
                    
            except Exception as e:
                result = f"❌ 스키마 조회 실패: {str(e)}"
            ...
            plan = await db_assistant.create_execution_plan(
                arguments["operation"],
                **arguments.get("parameters", {})
            )
            result = db_assistant._format_plan_display(plan)
        elif name == "confirm_and_execute":
            result = await db_assistant.confirm_and_execute(
            ...
            result = await db_assistant.analyze_slow_queries(
                arguments["database_secret"],
                arguments.get("limit", 10)
            )
        elif name == "get_table_io_stats":
            result = await db_assistant.get_table_io_stats(
                arguments["database_secret"],
                arguments.get("schema_name")
            )
        elif name == "get_index_usage_stats":
            result = await db_assistant.get_index_usage_stats(
                arguments["database_secret"],
                arguments.get("table_name")
            )
        elif name == "get_connection_stats":
            result = await db_assistant.get_connection_stats(
                arguments["database_secret"]
            )
        elif name == "get_memory_usage":
            result = await db_assistant.get_memory_usage(
                arguments["database_secret"]
            )
        elif name == "get_lock_analysis":
            result = await db_assistant.get_lock_analysis(
                arguments["database_secret"]
            )
        elif name == "get_replication_status":
            result = await db_assistant.get_replication_status(
                arguments["database_secret"]
            )
        elif name == "list_databases":
            db_info = await db_assistant.list_databases(
                arguments["database_secret"]
            )
            databases = db_info['databases']
