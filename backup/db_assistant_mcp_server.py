...
📋 **테이블 목록** ({len(tables_info)}개):"""
  ...
  🔹 **{table_name}** ({engine})
    ...
    async def validate_ddl(self, ddl_content: str, database_secret: Optional[str], filename: str) -> str:
        ...
        try:
            ...
            try:
                ...
                # DDL 유형에 따른 선택적 스키마 정보 추출
                relevant_schema_info = None
                if database_secret:
                    try:
                        relevant_schema_info = await self.extract_relevant_schema_info(ddl_content, database_secret, debug_log)
                        debug_log(f"관련 스키마 정보 추출 완료: {relevant_schema_info}")
                    except Exception as e:
                        debug_log(f"스키마 정보 추출 실패: {e}")
                
                claude_result = await self.validate_with_claude(ddl_content, database_secret, relevant_schema_info)
                debug_log(f"Claude 검증 결과 (전체): {claude_result}")
                debug_log(f"스키마 정보 전달됨: {relevant_schema_info is not None}")
                
                # Claude 응답이 "검증 통과"로 시작하면 성공으로 처리
                if claude_result.startswith("검증 통과"):
                    debug_log(f"Claude 검증 성공: {claude_result}")
                elif "오류:" in claude_result:
                    # 실제 오류만 문제로 분류
                    issues.append(f"Claude 검증: {claude_result}")
                    debug_log("Claude 검증에서 오류 발견")
                else:
                    # 기존 응답 형식도 고려 (하위 호환성)
                    debug_log(f"Claude 검증 완료: {claude_result}")
    ...
    async def validate_with_claude(self, ddl_content: str, database_secret: str = None, schema_info: dict = None) -> str:
        """
        ...
        if schema_info:
            ...
            # 순서대로 정렬하여 처리
            sorted_items = sorted(schema_info.items(), key=lambda x: x[1].get('order', 0))
            ...
            for key, info in sorted_items:
                ...
                if info['type'] == 'table':
                    ...
                    if 'columns' in info:
                        # ALTER TABLE 케이스
                        columns_info = [f"{col['name']}({col['data_type']})" for col in info['columns']]
                        ...
                        if info.get('created_in_file'):
                            schema_details.append(f"  - 파일 내 생성됨: {info['created_in_file']}")
                        schema_details.append(f"  - 유효 존재: {info.get('effective_exists', info['exists'])}")
                        
                        if info['exists'] and columns_info:
                            schema_details.append(f"  - 기존 컬럼: {', '.join(columns_info)}")
                        
                        if info.get('alter_type') and info.get('target_column'):
                            schema_details.append(f"  - ALTER 작업: {info['alter_type']} {info['target_column']}")
                    else:
                        # CREATE/DROP TABLE 케이스
                        action = "CREATE" if "CREATE" in key or info.get('exists') == False else "DROP"
                        schema_details.append(f"[{order}] {action} TABLE '{table_name}': DB존재={info['exists']}")
                    ...
                    if 'duplicate_column_indexes' in info:
                        # CREATE INDEX 케이스
                        schema_details.append(f"[{order}] CREATE INDEX '{index_name}' on '{table_name}':")
                        schema_details.append(f"  - 테이블 DB존재: {info['table_exists']}")
                        if info.get('created_in_file'):
                            schema_details.append(f"  - 테이블 파일내생성: {info['created_in_file']}")
                        schema_details.append(f"  - 테이블 유효존재: {info.get('effective_exists', info['table_exists'])}")
                        schema_details.append(f"  - 생성할 컬럼: {', '.join(info['target_columns'])}")
                        
                        # DB의 중복 인덱스
                        if info['duplicate_column_indexes']:
                            schema_details.append("  - DB의 동일 컬럼 구성 인덱스:")
                            for dup_idx in info['duplicate_column_indexes']:
                                schema_details.append(f"    * {dup_idx['name']} ({dup_idx['columns']}) - {'UNIQUE' if dup_idx['unique'] else 'NON-UNIQUE'}")
                        
                        # 파일 내 중복 인덱스
                        if info.get('file_duplicate_indexes'):
                            schema_details.append("  - 파일 내 동일 컬럼 구성 인덱스:")
                            for dup_idx in info['file_duplicate_indexes']:
                                schema_details.append(f"    * [{dup_idx['order']}] {dup_idx['name']} ({','.join(dup_idx['columns'])})")
                        
                        if not info['duplicate_column_indexes'] and not info.get('file_duplicate_indexes'):
                            schema_details.append("  - 중복 컬럼 구성 인덱스 없음")
                    else:
                        # DROP INDEX 케이스
                        schema_details.append(f"[{order}] DROP INDEX '{index_name}' on '{table_name}':")
                        schema_details.append(f"  - 테이블 존재: {info['table_exists']}")
                        schema_details.append(f"  - 인덱스 존재: {info['index_exists']}")
...
"""
    ...
    async def extract_current_schema_info(self, database_secret: str, use_ssh_tunnel: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"스키마 정보 추출 완료: {len(schema_info['tables'])}개 테이블, {len(schema_info['columns'])}개 테이블의 컬럼 정보")
            return schema_info
    ...
    async def extract_relevant_schema_info(self, ddl_content: str, database_secret: str, debug_log, use_ssh_tunnel: bool = True) -> Dict[str, Any]:
        """DDL 유형에 따라 관련된 스키마 정보만 추출 (순서 고려)"""
        try:
            # DDL 유형 파싱
            ...
            for i, ddl_statement in enumerate(ddl_info):
                ...
                if ddl_type == 'CREATE_TABLE':
                    ...
                    debug_log(f"CREATE TABLE [{i}] - {table_name} 존재: {table_exists}, 파일 내 생성 추가")
                    ...
                    debug_log(f"ALTER TABLE [{i}] - {table_name} DB존재: {table_exists}, 파일내생성: {created_in_file}, 유효존재: {effective_exists}")
                    ...
                    target_columns = [col.strip().strip('`').lower() for col in columns_str.split(',')]
                    ...
                    debug_log(f"CREATE INDEX [{i}] - {table_name}.{index_name}, DB중복: {len(duplicate_column_indexes)}개, 파일중복: {len(file_duplicate_indexes)}개")
                    ...
                    debug_log(f"DROP INDEX [{i}] - {table_name}.{index_name} 존재: {index_exists}")
        ...
        try:
            ...
            for ddl_statement in ddl_info:
                ...
                if ddl_type == 'CREATE_TABLE':
                    ...
                    result = await self.validate_drop_table_with_debug(cursor, ddl_statement, debug_log)
    ...
    def parse_ddl_detailed_with_debug(self, ddl_content: str, debug_log) -> List[Dict[str, Any]]:
        ...
        # ALTER TABLE 파싱 (상세)
        # ADD COLUMN
        alter_add_pattern = r'ALTER\s+TABLE\s+`?(\w+)`?\s+ADD\s+(?:COLUMN\s+)?`?(\w+)`?\s+([^,;]+)'
        ...
        for table_name, column_name, column_def in alter_add_matches:
            ddl_statements.append({
                'type': 'ALTER_TABLE',
                'table': table_name.lower(),
            ...
            debug_log(f"ALTER TABLE ADD COLUMN 구문 추가됨: {table_name}.{column_name}")
        
        # MODIFY COLUMN
        alter_modify_pattern = r'ALTER\s+TABLE\s+`?(\w+)`?\s+MODIFY\s+(?:COLUMN\s+)?`?(\w+)`?\s+([^,;]+)'
        alter_modify_matches = re.findall(alter_modify_pattern, ddl_content, re.IGNORECASE)
        ...
        for table_name, column_name, column_def in alter_modify_matches:
            ...
            debug_log(f"ALTER TABLE MODIFY COLUMN 구문 추가됨: {table_name}.{column_name}")
        
        # DROP COLUMN
        alter_drop_pattern = r'ALTER\s+TABLE\s+`?(\w+)`?\s+DROP\s+(?:COLUMN\s+)?`?(\w+)`?'
        ...
        for table_name, column_name in alter_drop_matches:
            ...
            debug_log(f"ALTER TABLE DROP COLUMN 구문 추가됨: {table_name}.{column_name}")
        ...
        for stmt in ddl_statements:
            if stmt['type'] == 'ALTER_TABLE':
                processed_tables.add(stmt['table'])
        
        for table_name in alter_table_matches:
            if table_name.lower() not in processed_tables:
                ddl_statements.append({
                    'type': 'ALTER_TABLE',
                    'table': table_name.lower(),
                    'alter_type': 'GENERAL'
                })
                debug_log(f"일반 ALTER TABLE 구문 추가됨: {table_name}")
    ...
    async def validate_create_index_with_debug(self, cursor, ddl_statement: Dict[str, Any], debug_log) -> Dict[str, Any]:
        ...
        if not table_exists:
            issues.append(f"테이블 '{table_name}'이 존재하지 않습니다. CREATE INDEX를 실행할 수 없습니다.")
            ...
            # 인덱스 컬럼 파싱
            index_columns = [col.strip().strip('`').lower() for col in columns.split(',')]
            ...
            if not issues:  # 기본 검증을 통과한 경우에만
                ...
                for idx_name, idx_columns in existing_index_columns.items():
                    if idx_columns == target_columns:
                        issues.append(f"동일한 컬럼 구성의 인덱스 '{idx_name}'이 이미 존재합니다.")
    ...
    async def validate_drop_index_with_debug(self, cursor, ddl_statement: Dict[str, Any], debug_log) -> Dict[str, Any]:
        ...
        if not table_exists:
            issues.append(f"테이블 '{table_name}'이 존재하지 않습니다. DROP INDEX를 실행할 수 없습니다.")
            debug_log(f"오류: 테이블 '{table_name}' 존재하지 않음")
        else:
            # 인덱스 존재 여부 확인
            cursor.execute("""
    ...
    async def validate_create_table_with_debug(self, cursor, ddl_statement: Dict[str, Any], debug_log) -> Dict[str, Any]:
        ...
        if table_exists:
            issues.append(f"테이블 '{table_name}'이 이미 존재합니다. CREATE TABLE은 실행할 수 없습니다.")
    ...
    async def validate_alter_table_with_debug(self, cursor, ddl_statement: Dict[str, Any], debug_log) -> Dict[str, Any]:
        ...
        if not table_exists:
            issues.append(f"테이블 '{table_name}'이 존재하지 않습니다. ALTER TABLE을 실행할 수 없습니다.")
            debug_log(f"오류: 테이블 '{table_name}' 존재하지 않음")
        ...
        if alter_type == 'ADD_COLUMN':
            ...
            if column_name and column_name in existing_columns:
                issues.append(f"컬럼 '{column_name}'이 이미 존재합니다. ADD COLUMN을 실행할 수 없습니다.")
            ...
            if column_name and column_name not in existing_columns:
                issues.append(f"컬럼 '{column_name}'이 존재하지 않습니다. DROP COLUMN을 실행할 수 없습니다.")
            ...
            if column_name and column_name not in existing_columns:
                issues.append(f"컬럼 '{column_name}'이 존재하지 않습니다. MODIFY COLUMN을 실행할 수 없습니다.")
                debug_log(f"오류: 컬럼 '{column_name}' 존재하지 않음")
            else:
    ...
    async def validate_drop_table_with_debug(self, cursor, ddl_statement: Dict[str, Any], debug_log) -> Dict[str, Any]:
        ...
        if not table_exists:
            issues.append(f"테이블 '{table_name}'이 존재하지 않습니다. DROP TABLE을 실행할 수 없습니다.")
            debug_log(f"오류: 테이블 '{table_name}' 존재하지 않음")
        else:
        ...
        result = {
            ...
            "details": {
                "table_exists": table_exists,
                "can_drop": table_exists
            }
        }
        
        debug_log(f"DROP TABLE 검증 완료: issues={len(issues)}, valid={len(issues) == 0}")
        debug_log(f"최종 결과: {result}")
...
async def handle_call_tool(name: str, arguments: dict) -> list[types.TextContent]:
    ...
    def validate_column_type_compatibility(self, existing_column: Dict[str, Any], new_definition: str, debug_log) -> Dict[str, Any]:
        """컬럼 데이터 타입 호환성 검증"""
        debug_log(f"타입 호환성 검증 시작: 기존={existing_column['data_type']}, 새로운={new_definition}")
        ...
        incompatible_changes = [
            # 문자열 -> 숫자
            (['VARCHAR', 'CHAR', 'TEXT', 'LONGTEXT', 'MEDIUMTEXT'], ['INT', 'BIGINT', 'DECIMAL', 'FLOAT', 'DOUBLE']),
            # 숫자 -> 문자열 (데이터 손실 가능)
            (['INT', 'BIGINT', 'DECIMAL', 'FLOAT', 'DOUBLE'], ['VARCHAR', 'CHAR']),
            # 날짜/시간 타입 변경
            (['DATE', 'DATETIME', 'TIMESTAMP'], ['INT', 'VARCHAR', 'CHAR']),
        ...
        if existing_type == 'DECIMAL' and new_type_info['type'] == 'DECIMAL':
            ...
            if (existing_precision and new_precision and new_precision < existing_precision) or \
               (existing_scale and new_scale and new_scale < existing_scale):
                issues.append(f"DECIMAL 정밀도를 ({existing_precision},{existing_scale})에서 ({new_precision},{new_scale})로 축소하는 것은 데이터 손실을 야기할 수 있습니다.")
                debug_log(f"정밀도 축소 문제: ({existing_precision},{existing_scale}) -> ({new_precision},{new_scale})")
        ...
        debug_log(f"타입 호환성 검증 완료: compatible={result['compatible']}, issues={len(issues)}")
        return result
