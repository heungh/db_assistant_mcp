...
class DDLValidationQCLIServer:
    ...
    async def generate_final_report(self, state: dict) -> str:
        ...
        if state.get("db_connection_info"):
            db_info = state["db_connection_info"]
            if db_info.get("success"):
                ...
                <div class="info-section">
                    ...
                    <table class="info-table">
                        ...
                        <tr><td><strong>서버 버전</strong></td><td>{db_info.get('server_version', 'N/A')}</td></tr>
                        <tr><td><strong>현재 데이터베이스</strong></td><td>{db_info.get('current_database', 'N/A')}</td></tr>
                        <tr><td><strong>연결 방식</strong></td><td>{db_info.get('connection_method', 'N/A')}</td></tr>
                    </table>
                </div>
                """
            else:
                ...
                <div class="info-section">
                    ...
                    <table class="info-table">
                        ...
                        <tr><td><strong>오류</strong></td><td>{db_info.get('error', 'N/A')}</td></tr>
                    </table>
                </div>
                """
        ...
        if state.get("warnings"):
            ...
            for warning in state["warnings"]:
                warnings_section += f"<li>{warning}</li>"
            warnings_section += """
                </ul>
            </div>
            """
...
<head>
    ...
    <style>
        ...
        .issue-list, .issues-list, .warnings-list {{
            margin: 10px 0;
            padding-left: 20px;
        }}
        .issue-list li, .issues-list li, .warnings-list li {{
            margin: 5px 0;
        }}
        .status-success {{
            color: #28a745;
    ...
    <div class="container">
        ...
        <div class="content">
            <div class="summary-grid">
                ...
                <div class="summary-item">
                    <h4>🔧 DDL 타입</h4>
                    <p>{state.get('ddl_type', 'UNKNOWN')}</p>
                </div>
                <div class="summary-item">
                    <h4>🗄️ 데이터베이스</h4>
                    <p>{state.get('database_secret', 'N/A')}</p>
                </div>
            </div>
        ...
        <div class="footer">
            <p>Generated by DDL Validation Q CLI MCP Server (Edge Connected)</p>
