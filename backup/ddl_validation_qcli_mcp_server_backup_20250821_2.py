...
class DBAssistantMCPServer:
    ...
    async def execute_explain_individual_queries(self, sql_content: str, cursor, debug_log):
        ...
        try:
            ...
            for i, stmt in enumerate(statements):
                ...
                # DDL 구문은 EXPLAIN 스킵
                if re.match(r'^\s*(CREATE|ALTER|DROP|INSERT|UPDATE|DELETE)', cleaned_stmt, re.IGNORECASE):
                    debug_log(f"쿼리 {i+1}: DDL/DML 구문이므로 EXPLAIN 스킵")
                    continue
                ...
                references_new_table = any(table in created_tables for table in query_tables)
                
                if references_new_table:
                    debug_log(f"쿼리 {i+1}: 새로 생성되는 테이블을 참조하므로 EXPLAIN 스킵 - 테이블: {[t for t in query_tables if t in created_tables]}")
                    continue
                ...
                try:
                    ...
                    result["explain_data"].append({
                        'query_index': i+1,
                        'query': cleaned_stmt[:200] + '...' if len(cleaned_stmt) > 200 else cleaned_stmt,
                    ...
                    if "Unknown column" in str(e):
                        result["issues"].append(f"쿼리 {i+1}에서 존재하지 않는 컬럼을 참조합니다: {cleaned_stmt[:100]}...")

            debug_log(f"개별 쿼리 EXPLAIN 완료: {len(result['explain_data'])}개 성공, {len(result['issues'])}개 오류")
            return result
    ...
    async def test_individual_query_validation(self, database_secret: str, filename: str) -> str:
        """개별 쿼리 검증 테스트 함수"""
        try:
            # SQL 파일 읽기
            ...
            # 데이터베이스 연결
            connection, tunnel_used = self.get_db_connection(database_secret, self.selected_database)
            if not connection:
                return "❌ 데이터베이스 연결 실패"
            ...
            # 개별 쿼리 검증 실행
            result = await self.execute_explain_individual_queries(sql_content, cursor, print)
    ...
    def extract_created_indexes(self, sql_content: str) -> List[str]:
        ...
        # CREATE INDEX 패턴
        index_pattern = r"CREATE\s+(?:UNIQUE\s+)?INDEX\s+`?([a-zA-Z_][a-zA-Z0-9_]*)`?\s+ON"
    ...
    async def validate_ddl(
        ...
        try:
                ...
                try:
                    ...
                    if cursor:
                        ...
                        # DQL(DML) 검증
                            ...
                            debug_log(f"개별 쿼리 EXPLAIN 함수 호출 완료: {explain_result}")
            ...
            try:
                ...
                if database_secret:
                    try:
                        ...
                        if explain_result:
                            explain_info_str = f"EXPLAIN 분석 결과: {str(explain_result)}"
                            debug_log(f"EXPLAIN 정보 문자열 변환 완료: {explain_info_str}")
            ...
            if dml_column_issues:
                ...
                debug_log(f"DML 컬럼 이슈 {len(dml_column_issues)}개를 최종 이슈 목록에 추가")
...
"""
            ...
            for i, stmt in enumerate(statements):
                ...
                if any(keyword in stmt_lower for keyword in ['select', 'update', 'delete']):
                    ...
                    for table_ref, column_name in column_refs:
                        ...
                        if actual_table in all_tables or actual_table in schema_cache or table_exists(actual_table):
    ...
    def parse_ddl_detailed_with_debug(
        ...
        alter_table_matches = re.findall(
            alter_table_pattern, clean_sql, re.IGNORECASE
        )
        ...
        for index_name, table_name, columns in create_index_matches:
            ...
            debug_log(
                f"CREATE INDEX 구문 추가됨: {index_name} on {table_name}({columns})"
            )
    ...
    async def generate_html_report(
        ...
        try:
            with open("/Users/heungh/Documents/SA/05.Project/01.Infra-Assistant/01.DB-Assistant/output/html_debug.txt", "a", encoding="utf-8") as f:
                f.write(f"HTML 생성 함수 호출됨 - claude_analysis_result: {claude_analysis_result}\n")
                f.flush()
        except:
            pass
        try:
